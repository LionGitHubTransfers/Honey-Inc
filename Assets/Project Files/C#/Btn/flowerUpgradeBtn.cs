using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.EventSystems;

public class flowerUpgradeBtn : MonoBehaviour, IPointerDownHandler // required interface when using 
{

    [SerializeField]
    private int _upGradePrice;

    [SerializeField]
    TMPro.TextMeshProUGUI capaCityLevelText, _upGradePriceText;

    [SerializeField]
    private int _flowerLevel;



    // Start is called before the first frame update
    void Start()
    {
        _flowerLevel = GameManager.gameManager.currentFlowerLevel;
        if (_flowerLevel > 9)
        {
            capaCityLevelText.text = "Level " + _flowerLevel;
        }
        else
        {
            capaCityLevelText.text = "Level 0" + _flowerLevel;
        }

        if (_flowerLevel > 0)
        {
            _upGradePrice = GameManager.gameManager.flowerUpgradePrice * (_flowerLevel + 1);
            _upGradePriceText.text = "$" + _upGradePrice;
            Debug.Log("Upgrade Price " + _upGradePrice + "Speed Level " + _flowerLevel);
        }
        else
        {
            _upGradePrice = GameManager.gameManager.flowerUpgradePrice;
            _upGradePriceText.text = "$" + _upGradePrice;
        }

        Debug.Log("Upgrade Price " + _upGradePrice);


    }



    public void OnPointerDown(PointerEventData eventData)
    {



        if (GameManager.gameManager.toalCash >= _upGradePrice)
        {

            GameManager.gameManager.SubtractCash(_upGradePrice);

            GameObject MoneyFx = Instantiate(GameManager.gameManager.MoneyBlast, PlayerController.playerController.transform.position, Quaternion.identity);
            MoneyFx.transform.localScale = new Vector3(1.2f, 1.2f, 1.2f);
            MoneyFx.transform.transform.position = new Vector3(MoneyFx.transform.position.x, 0.8f, 10.56f);


            GameManager.gameManager.UpgradeFlowerLevel();




            _flowerLevel = GameManager.gameManager.currentFlowerLevel;
            if (_flowerLevel > 9)
            {
                capaCityLevelText.text = "Level " + _flowerLevel;
            }
            else
            {
                capaCityLevelText.text = "Level 0" + _flowerLevel;
            }


            if (_flowerLevel > 0)
            {
                _upGradePrice = GameManager.gameManager.flowerUpgradePrice * (_flowerLevel + 1);
                _upGradePriceText.text = "$" + _upGradePrice;
                Debug.Log("Upgrade Price " + _upGradePrice + "Speed Level " + _flowerLevel);
            }
            else
            {
                _upGradePrice = GameManager.gameManager.flowerUpgradePrice;
                _upGradePriceText.text = "$" + _upGradePrice;
            }


            Debug.Log("Upgrade Price " + _upGradePrice);



        }
        else
        {

            Debug.Log("Not Enough Money ");
        }

        // throw new System.NotImplementedException();
    }

}
